const APP_VERSION = "20260209r1";
const CACHE_PREFIX = "devdannyt";
...
self.addEventListener("activate", (event) => {
  event.waitUntil((async () => {
    ...
    await Promise.all(
      keys.map((key) => {
        // delete older caches from THIS site
        if (key !== CACHE_NAME && key.startsWith(`${CACHE_PREFIX}-`)) {
          return caches.delete(key);
        }
        // also delete old caches from the original template
        if (key.startsWith("yalokgar-")) {
          return caches.delete(key);
        }
        return null;
      })
